...
static ssize_t tvp_region_show(struct device *dev,
...
static ssize_t
frame_format_show(struct device *dev,
		  struct device_attribute *attr, char *buf)
{
	int ret = 0;
...
static DEVICE_ATTR(frame_format, 0444, frame_format_show, NULL);
static DEVICE_ATTR(config, 0664, show_config, store_config);
static DEVICE_ATTR(debug, 0200, NULL, debug_store);
static DEVICE_ATTR(dump_pic, 0200, NULL, dump_pic_store);
static DEVICE_ATTR(log, 0664, show_log, store_log);
static DEVICE_ATTR(provider_vframe_status, 0444, provider_vframe_status_show, NULL);
static DEVICE_ATTR(tvp_region, 0444, tvp_region_show, NULL);
static DEVICE_ATTR(kpi_frame_num, 0664, show_kpi_frame_num,
	store_kpi_frame_num);
...
static int dim_probe(struct platform_device *pdev)
{
	...
	/* *********new********* */
	di_pdev->data_l = NULL;
	di_pdev->data_l = kzalloc(sizeof(*di_pdev->data_l), GFP_KERNEL);
	if (!di_pdev->data_l) {
...
void __exit dimini_module_exit(void)
{
