							  ...
							  struct file *file_vf)
{
	...
	if (!file_vf) {
		dp_buf_print(buf_mgr, PRINT_ERROR, "get_file_private_data fail\n");
		return NULL;
	}
	...
	if (!uhmod) {
		dp_buf_print(buf_mgr, PRINT_ERROR,
			 "uhmod is NULL:private_data=%px\n", file_vf->private_data);
		return NULL;
	}

	if (IS_ERR_VALUE(uhmod) || !uhmod->arg) {
		dp_buf_print(buf_mgr, PRINT_ERROR,
			 "dma file file_private_data is NULL uhmod=%px\n", uhmod);
		return NULL;
	}
					 ...
					 struct file *file_vf)
{
	...
	if (is_dec_vf) {
		...
		if (!vf) {
			dp_buf_print(buf_mgr, PRINT_ERROR, "vf is NULL.\n");
			return vf;
		}
...
int push_to_ref_list(struct dp_buf_mgr_t *buf_mgr, struct vframe_s *vf, struct file *file)
{
	...
	if (vf->type & VIDTYPE_INTERLACE) {
		...
		if (buf_mgr->ref_list_1 &&
			!(buf_mgr->ref_list_1->vf->type & VIDTYPE_INTERLACE)) {
			vf_ref->ref_other_number = 2;
			dp_buf_print(buf_mgr, PRINT_OTHER, "P->I\n");
		}

		if ((buf_mgr->ref_list_1 && buf_mgr->ref_list_1->vf->type & VIDTYPE_INTERLACE) &&
			(buf_mgr->ref_list_2 &&
			!(buf_mgr->ref_list_2->vf->type & VIDTYPE_INTERLACE))) {
			vf_ref->ref_other_number = 2;
	...
	buf_mgr->ref_list_2 = buf_mgr->ref_list_1;
...
static void print_all_in_ref_list(struct dp_buf_mgr_t *buf_mgr)
{
	...
	for (i = 0; i < VF_LIST_COUNT; i++) {
		if (atomic_read(&buf_mgr->ref_list[i].on_use)) {
			dp_buf_print(buf_mgr, PRINT_OTHER, " %d; %d/%d; %d, %px, %d; %d,%d\n",
...
void free_di_mgr_data(void *arg)
{
	...
	if (arg) {
		...
		if (!IS_ERR_OR_NULL(uvm_di_mgr->uhmod_dec))
			uvm_put_hook_mod((struct dma_buf *)(uvm_di_mgr->file->private_data),
				VF_SRC_DECODER);
...
int vf_ref_count_dec(struct dp_buf_mgr_t *buf_mgr, struct vf_ref_t *vf_ref)
{
	dp_buf_print(buf_mgr, PRINT_OTHER, "ref_count need -1: frame_index=%d, cur_count=%d\n",
		vf_ref->vf->frame_index, vf_ref->ref_count);
	...
	if (vf_ref->ref_count > 1) {
		...
		if (vf_ref->qbuf_id == buf_mgr->reset_id)
			...
			dp_buf_print(buf_mgr, PRINT_ERROR,
				"ref_dec, last instance qbuf, no recycle %d, %d\n",
...
int buf_mgr_dq_checkin(struct dp_buf_mgr_t *buf_mgr, struct file *file)
{
	...
	dp_buf_print(buf_mgr, PRINT_OTHER,
		"dq_checkin: frame_index=%d, %px, %px\n", vf->frame_index, buf_mgr, file);

	if (vf->type & VIDTYPE_V4L_EOS) {
		dp_buf_print(buf_mgr, PRINT_OTHER, "dq_checkin: eos\n");
		return -1;
	}
...
int buf_mgr_q_checkin(struct dp_buf_mgr_t *buf_mgr, struct file *file)
{
	...
	if (!vf) {
		dp_buf_print(buf_mgr, PRINT_OTHER,
			"q_checkin: not find vf, buf_mgr=%px, file=%px\n", buf_mgr, file);
		return -1;
	}

	dp_buf_print(buf_mgr, PRINT_OTHER,
		"q_checkin: frame_index=%d, %px, %px\n", vf->frame_index, buf_mgr, file);

	vf_ref = get_ref_from_list(buf_mgr, vf);
	if (!vf_ref) {
		dp_buf_print(buf_mgr, PRINT_ERROR, "q_checkin ref dec fail\n");
		return -1;
	}

	if (vf_ref->qbuf_id > 0) {
		dp_buf_print(buf_mgr, PRINT_ERROR, "q_checkin second:%d %d\n",
			vf_ref->qbuf_id, buf_mgr->reset_id);
		vf_ref->qbuf_id = buf_mgr->reset_id;
		return 0;
	}
	...
	if (!vf_ref->di_processed) {
		if (vf_ref->ref_other_number == 2) {
			...
			if (!vf_ref1) {
				dp_buf_print(buf_mgr, PRINT_ERROR, "q_checkin 0ref1 dec fail\n");
				return 0;
			}
			...
			vf_ref1 = get_ref1_from_list(buf_mgr, vf_ref);
			if (!vf_ref1) {
				dp_buf_print(buf_mgr, PRINT_ERROR, "q_checkin 1ref1 dec fail\n");
				return 0;
			}

			vf_ref2 = get_ref1_from_list(buf_mgr, vf_ref1);
			if (!vf_ref2) {
				dp_buf_print(buf_mgr, PRINT_ERROR, "q_checkin ref2 dec fail\n");
				return 0;
			}
...
int buf_mgr_free_checkin(struct dp_buf_mgr_t *buf_mgr, struct file *file)
{
	...
	vf = get_vf_from_file(buf_mgr, file);
	if (!vf) {
		dp_buf_print(buf_mgr, PRINT_ERROR, "free_checkin: not find vf\n");
		return -1;
	}

	dp_buf_print(buf_mgr, PRINT_OTHER, "free_checkin: frame_index=%d\n", vf->frame_index);

	vf_ref = get_ref_from_list(buf_mgr, vf);
	if (!vf_ref) {
		dp_buf_print(buf_mgr, PRINT_OTHER, "free_checkin ref dec fail\n");
		return -1;
	}
...
int di_processed_checkin(struct file *file)
{
	...
	dp_buf_print(buf_mgr, PRINT_OTHER, "processed: frame_index=%d\n", vf->frame_index);

	vf_ref = get_ref_from_list(buf_mgr, vf);
	if (!vf_ref) {
		dp_buf_print(buf_mgr, PRINT_ERROR, "processed:ref dec fail\n");
		return -1;
	}
	...
	if (vf_ref->ref_other_number > 1) {
		...
		if (!vf_ref1) {
			mutex_unlock(&buf_mgr->file_mutex);
			dp_buf_print(buf_mgr, PRINT_ERROR,
				"processed: ref1 dec fail, frame_index=%d\n", vf->frame_index);
			return -1;
		}
		if (!need_dec_two)
			vf_ref_count_dec(buf_mgr, vf_ref1);
	...
	if (need_dec_two) {
		...
		if (!vf_ref2) {
			mutex_unlock(&buf_mgr->file_mutex);
			dp_buf_print(buf_mgr, PRINT_ERROR,
				"processed: ref2 dec fail, frame_index=%d\n", vf->frame_index);
			return -1;
		}
		vf_ref_count_dec(buf_mgr, vf_ref2);
		vf_ref_count_dec(buf_mgr, vf_ref1);
	...
	if (vf_ref->di_processed)
		dp_buf_print(buf_mgr, PRINT_ERROR, "processed: already processed!!!\n");
